...
export const SessionTranscript: React.FC<SessionTranscriptProps> = ({ analyticsData }) => {
  ...
  return (
    <div className="mt-8">
      ...
      <div className="max-h-80 overflow-y-auto rounded-2xl bg-white/90 dark:bg-gray-800/90 shadow-[0_4px_24px_0_rgba(123,104,238,0.06)] dark:shadow-[0_4px_24px_0_rgba(123,104,238,0.1)] p-6">
        {dummyTranscripts.map((data, index) => (
          <div key={index} className="mb-3">
            <span className="text-[15px] text-gray-800 dark:text-gray-200">{data.transcript}</span>
          </div>
        ))}
      </div>
    </div>
