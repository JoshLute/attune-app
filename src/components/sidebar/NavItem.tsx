...
export function NavItem({ icon, label, href = "/", isActive = false, isRecording = false }: NavItemProps) {
  return (
    <Link to={href} className={cn(
      ...
      {isRecording && (
        <Circle 
          className="absolute right-3 w-2 h-2 text-red-500 animate-[pulse_1s_ease-in-out_infinite]" 
          fill="currentColor"
        />
      )}
    </Link>
